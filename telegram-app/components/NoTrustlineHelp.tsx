  ...
  async function handleAutoAdd() {
    ...
    try {
      ...
      if (!accountRes.ok) {
        ...
        fetch("/api/stellar/submit", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            publicKey,
            errorMessage: `Account fetch ${accountRes.status}: ${errBody?.title ?? "not found"}`,
            type: "trustline",
          }),
        }).catch(() => null);
        setStep("error");
        setErrorMsg(
          "Could not load account from Stellar network. Is the wallet funded?",
        );
        return;
      }
      ...
      // Submit via our server proxy (avoids CORS issues) — XDR only, key never sent
      const submitRes = await fetch("/api/stellar/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ xdr, publicKey, type: "trustline" }),
      });
      ...
      if (submitJson.success) {
        ...
        setStep("error");
        setErrorMsg(
          submitJson.error ?? "Transaction failed. Please try again.",
        );
      }
    } catch {
      setStep("error");
        ...
        <div className="glass-card border border-white/10 rounded-xl p-4 space-y-3">
          <div className="flex items-start space-x-2 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
            ...
            <p className="text-[10px] text-red-300 leading-relaxed">
              Your secret key is used <strong>locally only</strong> to sign the
              transaction. It is never sent to our servers or stored anywhere.
            </p>
          </div>
