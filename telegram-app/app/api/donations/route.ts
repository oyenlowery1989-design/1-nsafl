...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: donorRows, error: donorErr } = await supabase
      .from("donations")
      .select(
        "amount, wallet_id, donation_type, donation_target, wallets(stellar_address, users(telegram_username, telegram_first_name))",
      )
      .eq("verified", true)
      .order("created_at", { ascending: false });
    ...
    const walletTotals = new Map<
      string,
      { total: number; row: DonationRow; causes: string[] }
    >();

    for (const r of rows) {
      const wid = r.wallet_id;
      const cause =
        r.donation_type === "team" && r.donation_target
